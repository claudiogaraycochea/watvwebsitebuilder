...
import './ModuleFacebookComment.css';

class ModuleFacebookComment extends Component {
  constructor(props) {
    super(props);
    this.state = {
      moduleSrc: null,
      ...
      return (
        <div className="mod-link mod-row">
          <a href={this.props.moduleSrc.buttonLink} className="mod-btn mod-large-full mod-btn-no-space" style={styles.button}>
            <i className="mod-icon-facebook mod-icon-space" /> {(this.props.moduleSrc.buttonTitle!=='') ? this.props.moduleSrc.buttonTitle : 'Buy Now' }
          </a>
        </div>
      );
    }
...
export default ModuleFacebookComment;