...
import './ModuleWhatsapp.css';

class ModuleWhatsapp extends Component {
  constructor(props) {
    super(props);
    this.state = {
      moduleSrc: null,
    ...
    if(this.state.properties===true) {
      return (
        <div>
          ...
          <div className="">
            <input type="number" defaultValue={this.state.moduleSrc.buttonLink} className="inp" placeholder="Telephone Number (Ex: 19000000000)" onKeyUp={(e)=>{this.createModuleSrc(this.props,e,'buttonLink')}} />
          </div>
        </div>
      )
    }
      ...
      return (
        <div className="mod-link mod-row">
          <a href={`https://wa.me/${this.props.moduleSrc.buttonLink}`} className="mod-btn mod-large-full mod-btn-no-space" style={styles.button}>
            <i className="mod-icon-whatsapp mod-icon-space" /> {(this.props.moduleSrc.buttonTitle!=='') ? this.props.moduleSrc.buttonTitle : 'Whatsapp' }
          </a>
        </div>
      );
    }
